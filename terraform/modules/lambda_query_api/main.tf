locals {
  function_name = "${var.project_name}-${var.environment}-lambda-query-api"
}

data "archive_file" "package" {
  type        = "zip"
  source_dir  = "${path.module}/src"
  output_path = "${path.module}/build/lambda_query_api.zip"
}
...
resource "aws_lambda_function" "lambda_query" {
  ...
  environment {
    variables = {
      ...
      DEFAULT_EVENTS_LIMIT = tostring(var.default_events_limit)
      MAX_EVENTS_LIMIT     = tostring(var.max_events_limit)
    }
  }

  depends_on = [aws_iam_role_policy.lambda_query]
}
